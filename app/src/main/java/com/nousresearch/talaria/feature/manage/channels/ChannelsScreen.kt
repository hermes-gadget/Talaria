...
@Composable
fun ChannelsScreen(
    vm: SimpleManageViewModel = viewModel(factory = SimpleManageViewModel.factory { getChannels() }),
) {
    ...
    ScreenScaffold("Channels", "Messaging platforms", actions = {
        ...
        when {
            ...
            ui.error != null -> ErrorBox(ui.error!!, vm::refresh)
                ...
                val list = ui.data as List<MessagingPlatform>
                LazyColumn {
                    items(list, key = { it.id }) { p ->
                        Surface(Modifier.fillMaxWidth().padding(vertical = 4.dp), color = MaterialTheme.colorScheme.surfaceVariant) {
                            androidx.compose.foundation.layout.Column(modifier = Modifier.padding(12.dp)) {
                                ...
                                Text(p.description ?: "")
                                Text("state=${p.state} enabled=${p.enabled} configured=${p.configured}")
                                p.error_message?.let { Text(it, color = MaterialTheme.colorScheme.error) }
                            }
                        }
                    }
                }
