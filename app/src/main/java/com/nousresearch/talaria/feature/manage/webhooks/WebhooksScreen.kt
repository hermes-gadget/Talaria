...
@Composable
fun WebhooksScreen(
    vm: SimpleManageViewModel = viewModel(factory = SimpleManageViewModel.factory { getWebhooks() }),
) {
    ...
    ScreenScaffold("Webhooks", "Dynamic subscriptions", actions = {
        ...
        when {
            ...
            ui.error != null -> ErrorBox(ui.error!!, vm::refresh)
                ...
                val list = ui.data as List<WebhookRoute>
                LazyColumn {
                    items(list, key = { it.name }) { w ->
                        Surface(Modifier.fillMaxWidth().padding(vertical = 4.dp), color = MaterialTheme.colorScheme.surfaceVariant) {
                            androidx.compose.foundation.layout.Column(modifier = Modifier.padding(12.dp)) {
                                ...
                                Text(w.description ?: "")
                                Text(w.url ?: "")
                                Text("events=${w.events.joinToString()} enabled=${w.enabled}")
                            }
                        }
                    }
                }
